...
| 4 | **NW-MRC training-to-receive closed loop.** Firmware arms a packet-free noise window, services `NOISE_READY` into a per-branch noise estimate, then services the following packet's `TRAINING_DONE`, commits `w_k ∝ |h_k|/σ²_k`, and proves the live MRC combiner consumes active W. | SYSTEM | `ram_backdoor_nwmrc.core`, `ram_backdoor/test_grouper_trouper_nwmrc_e2e.py`, and RAM-linked `test_trouper_nwmrc_e2e.c`. | ✅ passed — native-IRQ production eigensolver image (2,968 B) booted through Grouper and drove active MRC output |
